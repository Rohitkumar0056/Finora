...
export const summaryAnalyticsService = async (
  ...
  const currentPeriodPipeline: PipelineStage[] = [
    ...
    },
    {
      $project: {
        ...
        // Savings percentage & expense ratio calculations
        savingData: {
          $let: {
            vars: {
            ...
            in: {
              // Saving % = ((Income - Expenses) / Income) * 100
              savingsPercentage: {
                $cond: [
                  { $lte: ["$$income", 0] },
                  0,
  ...
  // Default percentage change object
  ...
  if (from && to && rangeValue !== DateRangeEnum.ALL_TIME) {
    ...
    // If yearly preset → subtract 1 year, otherwise subtract days
    const isYearly = [
...
export const expensePieChartBreakdownService = async (
  ...
  const pipleline: PipelineStage[] = [
    ...
    // Split top 3 categories and group the rest into "others"
    {
      $facet: {
        topThree: [{ $limit: 3 }],
        others: [
