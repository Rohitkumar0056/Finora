...
import { planFeatures } from "../constants/subscription";
...
export const getUserSubscriptionStatusService = async (userId: string) => {
  ...
  // Prefer the exact stored subscription for the user instead of scanning a customer subscription list,
  // because canceled/trialing records can remain in the customer history and stale list results can
  // cause the UI to keep showing a trial as active after the user cancels it in Stripe.
  const subscriptionRecord = await SubscriptionModel.findOne({ userId: user._id })
    .sort({ updatedAt: -1, createdAt: -1 })
    .select("stripeSubscriptionId status trialEndsAt")
    .lean();

  if (user.stripeCustomerId || subscriptionRecord?.stripeSubscriptionId) {
    try {
      const stripeSubscriptions = user.stripeCustomerId
      ...
      const stripeSubscriptionId =
        bestStripeSubscription?.id ??
        subscriptionRecord?.stripeSubscriptionId ??
        null;

      if (stripeSubscriptionId) {
        const s = await stripeClient.subscriptions.retrieve(stripeSubscriptionId);
        const price = s.items.data[0]?.price;
        const priceId = typeof price === "string" ? price : price?.id;
        const plan = getPlanFromPriceId(priceId as string);
        const firstItem = s.items.data[0];
        const normalizedStatus = mapStripeStatus(
          s.status,
          s.trial_end ?? null,
          s.cancel_at ? true : Boolean(priceId)
        );
        const update = {
          userId: user._id,
        ...
        if (user.subscriptionId?.toString() !== doc._id.toString()) {
  ...
  const latestSub = await SubscriptionModel.findOne({ userId: user._id })
    .sort({ updatedAt: -1, createdAt: -1 })
    .lean();
  const subscriptionDoc = (latestSub || (user.subscriptionId as any)) as any;
  if (subscriptionDoc?._id && user.subscriptionId?.toString() !== subscriptionDoc._id.toString()) {
    user.subscriptionId = subscriptionDoc._id as any;
    await user.save();
  }
  // Backfill missing status for existing records created before schema had 'status'
  let statusValue = normalizeSubscriptionStatus(subscriptionDoc.status as any);
  if (!statusValue) {
  ...
  if (
    ...
    statusValue = SubscriptionStatus.TRIAL_EXPIRED;
    await SubscriptionModel.updateOne(
      { _id: (subscriptionDoc as any)._id },
...
function mapStripeStatus(
  ...
  switch (s) {
    ...
    default:
      return trialEnd ? SubscriptionStatus.TRIAL_EXPIRED : SubscriptionStatus.CANCELED;
  }
}
...
function normalizeSubscriptionStatus(
  ...
  const normalized = String(status).toLowerCase() as SubscriptionStatus;
...
export const upgradeToProSubscriptionService = async (
  ...
  // Check the latest subscription from DB, not the user's old reference
  const latestSub = await SubscriptionModel.findOne({ userId: user._id })
    .sort({ updatedAt: -1, createdAt: -1 })
    .lean();
  const latestStatus = latestSub ? normalizeSubscriptionStatus(latestSub.status) : null;
  if (latestStatus === SubscriptionStatus.ACTIVE) {
