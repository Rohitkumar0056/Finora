...
async function handleCustomerSubscriptionUpdated(
  ...
  if (!currentSub && stripeSubscription.status !== "active") {
    return;
  }
  ...
  const shouldSetActive = stripeSubscription.status === "active";
  ...
  if (shouldSetActive) {
    ...
    updatePayload.upgradedAt = new Date();
    updatePayload.canceledAt = null;
  }

  if (isPlanSwitch || shouldSetActive) {
    await SubscriptionModel.findOneAndUpdate(
      { userId },
      { $set: updatePayload },
      { upsert: true, new: true }
    ...
    console.log(
      shouldSetActive
        ? `✅ Subscription restored to active – user ${userId}`
